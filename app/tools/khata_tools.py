...
async def _find_customer(session, shop_id: str, name: str) -> Customer | None:
    stmt = select(Customer).where(
        Customer.shop_id == shop_id, func.lower(Customer.name) == name.strip().lower()
    )
...
async def add_credit(
    session, shop_id: str, customer_name: str, amount: float, related_bill_id: int | None = None
) -> Customer:
    ...
    not this function."""
    customer = await _find_customer(session, shop_id, customer_name)
    if customer is None:
        customer = Customer(shop_id=shop_id, name=customer_name)
...
async def khata_add(
    ...
    async with async_session_maker() as session:
        customer = await add_credit(session, shop_id, customer_name, amount, related_bill_id)
...
@tool
async def khata_pay(customer_name: str, amount: float, shop_id: Annotated[str, InjectedToolArg]) -> str:
...
@tool
async def khata_balance(customer_name: str, shop_id: Annotated[str, InjectedToolArg]) -> str:
